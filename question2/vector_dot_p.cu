...
int main(int argc, char * argv[]){
  ...
  // Output variables
  unsigned long N[3]= {100000000, 500000000, 1000000000};

  size_t size,size_V;
  float *V1sp,*V2sp; // Single precision
  double *V1dp,*V2dp; // double precision
  int thread = 8; // this has to be an input.
  ...
  if(argc == 3){
    ...
    if(strcmp(argv[1],"-p")==0 && strcmp(argv[2],"8")==0 && strcmp(argv[3],"-v")==0){
      // now verify the CPU version answers
      printf("%s\n",argv[1] );
    }
    if(argv[1] == "-c" && argv[1] == "8" && argv[1] == "-v"){
      // now verify the GPU version answers
    }
  }
  else if(argc == 1){
